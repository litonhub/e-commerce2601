...
const ExcelImport = () => {
    ...
    const handleSelectFile = (e) => {
        ...
        const valid = selected.name.endsWith(".xlsx") || selected.name.endsWith(".xls");
        if (!valid) {
            return toast.error("Please select an Excel (.xlsx/.xls) file.");
        }
        setFile(selected);
    };
    ...
    const handleImport = async () => {
        if (!file) {
            return toast.warning("Please select an Excel file.");
        }
    ...
    const handleFailedReport = async () => {
        ...
        try {
            ...
            const link = document.createElement("a");
            link.href = url;
            link.download = "failed-products.xlsx";
            link.click();
            window.URL.revokeObjectURL(url);
        } catch {
            ...
            <div className="rounded-2xl border border-brdr bg-white p-8 shadow-sm">
                ...
                <div className="mb-8 flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            Upload your .xlsx or .xls file to bulk import products.
                        </p>
                    </div>
                    <button
                        type="button"
                ...
                <div className="mx-auto max-w-2xl">
                    {!file ? (
                        <label className="flex h-48 w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-brdr bg-primary/5 transition-all hover:border-primary hover:bg-primary/10">
                            <div className="flex flex-col items-center justify-center pt-5 pb-6 text-primary">
                                <FileSpreadsheet className="mb-3 h-12 w-12 opacity-80" />
                                <p className="text-base font-semibold">Click to upload Excel File</p>
                                <p className="mt-1 text-sm text-gray-500">.xlsx or .xls only</p>
                            </div>
                            <input
                                ref={fileInputRef}
                                type="file"
                    ...
                    <div className="mt-6 flex justify-center">
                        ...
                        >
                            <UploadCloud size={18} />
                            {loading ? "Importing Data..." : "Import Excel"}
                        </button>
                    </div>
                </div>
            </div>
